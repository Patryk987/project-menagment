...
class SimpleWAF
{
    var $log_file = "waf-logs";
    var $telegram_notice = true;
    var $telegram_chatid = "https://t.me/+rZyzjeZSJslmZjlk";
    ...
    {
        $f = fopen(__DIR__ . '/../logger/' . $this->log_file . '.txt', 'a');
        ...
        if ($this->protect_upload == false) {
            ...
            foreach ($f_uploaded as $key => $value) {
                ...
                if ($f_uploaded[$key]['size'] > 67 && $f_uploaded[$key]['size'] < 129) {
                    ...
                    if (
                        preg_match('`^X5O!P%@AP' . '\[4\\\PZX54\(P\^\)7CC\)7}\$EIC' .
    ...
    private function fetch_uploads()
    {
        ...
        foreach ($_FILES as $f_key => $f_value) {
            foreach ($f_value as $prop_key => $prop_value) {
                // Fetch all but 'error':
                if (!in_array($prop_key, array('name', 'type', 'tmp_name', 'size'))) {
                    continue;
                }
                $file_buffer = $f_key;
    ...
    public function secureMe($activate)
    {
        ...
        if ($activate == true) {
            ...
            if ($this->protect_union_sql === true) {
                ...
                if ($this->get_query_string() != $check)
                    $stop++;
                if (preg_match($regex_union, $this->get_query_string()))
                    $stop++;
                if (preg_match('/([OdWo5NIbpuU4V2iJT0n]{5}) /', rawurldecode($this->get_query_string())))
                    $stop++;
                if (strstr(rawurldecode($this->get_query_string()), '*'))
                    $stop++;
                if (!empty($stop)) {
